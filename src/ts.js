...
Timeseries.prototype.dynamicTimeWarp = function(other)
{
	// tolerated difference before considering two values not identical (normalized values)
	var TOLERANCE = .125;

	var N = this.series.length;
	var M = other.series.length;
	var s = this.series;
	var t = other.series;
	...
	for (var i = 1; i <= N; i++) 
	{
		for (var j = 1; j <= M; j++) 
		{
			var cost = Math.abs(s[i-1] - t[j-1]) > TOLERANCE ? 1 : 0;
...
Timeseries.prototype.distance = Timeseries.prototype.dynamicTimeWarp; //distanceEDR;
